...
export default function AddToCart({ id, title, price, image, slug, inStock }: Props) {
  ...
  if (qty === 0) {
    return <Button onClick={() => addItem({ id, title, price, image, slug })} size='lg' className='mt-10 w-full'>Add to Cart</Button>
  }
  ...
  return (
    <div className='mt-10'>
      <div className='flex w-full items-center justify-center gap-4 rounded-full border border-neutral-200 py-3'>
        <Button variant='ghost' size='sm' onClick={() => updateQuantity(id, qty - 1)} className='h-8 w-8 p-0 text-lg'>−</Button>
        <Text as='span' className='w-8 text-center text-lg font-semibold text-neutral-900'>{qty}</Text>
        <Button variant='ghost' size='sm' onClick={handleIncrease} disabled={qty >= 2} className='h-8 w-8 p-0 text-lg'>+</Button>
      </div>
